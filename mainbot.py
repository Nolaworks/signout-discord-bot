...
def admin_user(user):
    ...
    print(f"User: {user.name}, Roles: {[role.name for role in user.roles]}")
...
async def signout(interaction: discord.Interaction, time: str):
    ...
    data = load_tools()
    if tool not in data["tools"]:
        data["tools"][tool] = {"max_time_hours": 168, "reservations": []}
        save_tools(data)
...
async def on_guild_channel_create(channel):
    if isinstance(channel, discord.TextChannel) and channel.name.startswith("signout-"):
        ...
        if tool_name not in data["tools"]:
            data["tools"][tool_name] = {"max_time_hours": 168, "reservations": []}
            save_tools(data)
