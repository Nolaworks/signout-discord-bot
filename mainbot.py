...
def parse_time_with_gpt(time_str):
    """Uses ChatGPT to parse a user-provided time string into a standard format."""
    prompt = f"""
...
def is_tool_available(tool_name, requested_time):
    """Checks if the tool is available at the requested time."""
    data = load_tools()
...
async def clean_expired_signouts():
    ...
    for tool, reservations in data["tools"].items():
        data["tools"][tool] = [r for r in reservations if datetime.datetime.strptime(r["time"], "%Y-%m-%d %H:%M") > now]
    save_tools(data)

@bot.tree.command(name="signout", description="Sign out a tool at a specific time")
@app_commands.describe(tool="Tool name", time="Any format (e.g., 'tomorrow 3pm', 'next Friday')")
async def signout(interaction: discord.Interaction, tool: str, time: str):
    data = load_tools()
    formatted_time = parse_time_with_gpt(time)
...
async def return_tool(interaction: discord.Interaction, tool: str):
    data = load_tools()
...
async def reservations(interaction: discord.Interaction, tool: str = None):
    ...
    if tool:
        ...
        all_reservations = [f"**{t}:**\n" + "\n".join([f"- {r['user']} at {r['time']}" for r in res]) for t, res in data["tools"].items() if res]
